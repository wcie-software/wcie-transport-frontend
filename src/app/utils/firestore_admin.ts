...
export class FirestoreAdminHelper {
	...
	async getCollection<Type>(
		...
		const results = await (orderByField
		...
		if (!results.empty) {
			...
			results.forEach(doc => {
				...
				if (parsed.success) {
					...
					console.warn(`Failed to parse '${collectionName}/${doc.id}' data:`, parsed.error);
				}
			});
	...
	async getDocument<Type>(
		...
		const doc = await this.getDocuments<Type>(collectionName, schema, [documentId]);
		if (doc.length > 0) {
			return doc[0];
		} else {
			return undefined;
